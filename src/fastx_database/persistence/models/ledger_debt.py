...
Usage:
    >>> from fastx_database.persistence.models.ledger_debt import LedgerDebt, LedgerDebtPayment, LedgerDebtCredit
"""
...
class LedgerDebt(Base):
    ...
    client_debt_id = Column(String(StringLength.URN), nullable=False)
    name = Column(String(StringLength.DESCRIPTION), nullable=False)
    direction = Column(String(StringLength.STATUS), nullable=False)  # THEY_OWE_ME | I_OWE_THEM
    initial_amount = Column(Numeric(18, 6), nullable=False)
    currency = Column(String(StringLength.CODE), nullable=False, default=Currency.INR)
    due_date = Column(Date, nullable=True)
    note = Column(Text, nullable=True)
    phone = Column(String(StringLength.PHONE), nullable=True)
    created_at = Column(
...
class LedgerDebtPayment(Base):
    ...
    client_debt_payment_id = Column(String(StringLength.URN), nullable=False)
    client_debt_id = Column(String(StringLength.URN), nullable=False, index=True)
...
class LedgerDebtCredit(Base):
    ...
    client_debt_credit_id = Column(String(StringLength.URN), nullable=False)
    client_debt_id = Column(String(StringLength.URN), nullable=False, index=True)
    amount = Column(Numeric(18, 6), nullable=False)
