...
class UserMfaFactor(Base):
    ...
    factor_type = Column(String(StringLength.STATUS), nullable=False, index=True)
    label = Column(String(StringLength.URN), nullable=False, default="Authenticator")
    credential_data = Column(Text, nullable=True)
    webauthn_credential_id = Column(String(StringLength.DESCRIPTION), nullable=True, index=True)
