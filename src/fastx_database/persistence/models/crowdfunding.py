...
class CrowdfundingCampaign(Base, TimestampMixin, SoftDeleteMixin):
    ...
    title = Column(String(StringLength.DESCRIPTION), nullable=False)
    slug = Column(String(StringLength.NAME), nullable=False, index=True)
    short_description = Column(String(StringLength.LONG_TEXT), nullable=True)
    body = Column(Text, nullable=True)
    goal_amount_cents = Column(BigInteger, nullable=False)
    funded_amount_cents = Column(BigInteger, nullable=False, default=0)
    currency = Column(String(StringLength.CODE), nullable=False, default=Currency.USD)
    status = Column(
        String(StringLength.STATUS),
        nullable=False,
    ...
    category = Column(String(StringLength.URN), nullable=True, index=True)
...
class CrowdfundingReward(Base, TimestampMixin, SoftDeleteMixin):
    ...
    delivery_estimate = Column(String(StringLength.DESCRIPTION), nullable=True)
...
class CrowdfundingPledge(Base, TimestampMixin):
    ...
    payment_provider_ref = Column(String(StringLength.NAME), nullable=True, index=True)
    idempotency_key = Column(String(StringLength.URN), nullable=True, unique=True)
