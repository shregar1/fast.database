...
Usage:
    >>> from fastx_database.persistence.models.messaging_chat import Chat, ChatMessage
"""
...
class Chat(Base):
    ...
    kind = Column(String(StringLength.STATUS), nullable=False, index=True)
    title = Column(String(StringLength.DESCRIPTION), nullable=True)
...
class ChatParticipant(Base):
    ...
    role = Column(String(StringLength.STATUS), nullable=False, default="member", index=True)
    notification_level = Column(String(StringLength.STATUS), nullable=False, default="all", index=True)
...
class ChatMessage(Base):
    ...
    content_type = Column(String(StringLength.STATUS), nullable=False, default="text", index=True)
...
class ChatMessageNotification(Base):
    ...
    channel = Column(String(StringLength.STATUS), nullable=False, index=True)
    status = Column(String(StringLength.STATUS), nullable=False, default=Status.PENDING, index=True)
    provider_message_id = Column(String(StringLength.NAME), nullable=True, index=True)
    error_message = Column(String(StringLength.LONG_TEXT), nullable=True)
    created_at = Column(
        DateTime(timezone=True), nullable=False, default=datetime.utcnow
    )
    updated_at = Column(
