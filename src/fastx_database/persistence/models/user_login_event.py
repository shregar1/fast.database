...
from fastx_database.core.constants.db import MAX_IP_ADDRESS_LENGTH, StringLength
from fastx_database.core.constants.status import AuthMethod
...
class UserLoginEvent(Base):
    ...
    email_attempt = Column(String(StringLength.EMAIL), nullable=True, index=True)
    ip_address = Column(String(MAX_IP_ADDRESS_LENGTH), nullable=True, index=True)
    user_agent = Column(Text, nullable=True)
    success = Column(Boolean, nullable=False, index=True)
    failure_reason = Column(String(StringLength.CODE), nullable=True)
    auth_method = Column(String(StringLength.STATUS), nullable=False, default=AuthMethod.PASSWORD, index=True)
    created_at = Column(
