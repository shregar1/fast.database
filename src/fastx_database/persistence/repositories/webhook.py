...
Usage:
    >>> from fastx_database.persistence.repositories.webhook import WebhookRepository
...
from fastx_database.core.constants.db import DEFAULT_ATTEMPTS
...
class WebhookDeliveryRepository(IRepository):
    ...
    def list_recent(
        ...
        skip: int = 0,
        limit: int = Pagination.SMALL_LIMIT,
    ) -> tuple[list[dict], int]:
